...
function Table(boot) {
    ...
    this.addPlayer = function(player, client) {
        if (this.getActivePlayers() <= maxPlayers) {
            // Check if player has enough chips to play (at least boot amount)
            var minChipsRequired = tableInfo ? tableInfo.boot : (boot || 1);
            if (player.playerInfo.chips < minChipsRequired) {
                console.log('Player rejected - insufficient chips:', player.playerInfo.chips, 'Required:', minChipsRequired);
                return false;
            }
    ...
    this.placeBetOnly = function(id, bet, blind) {
        ...
        if (players[id].playerInfo.chips < bet) {
            console.log('Insufficient chips for bet. Required:', bet, 'Available:', players[id].playerInfo.chips);
            return false;
        }
        ...
        DAL.db.collection('users').updateOne({
    ...
    this.placeBet = function(id, bet, blind) {
        var result = this.placeBetOnly(id, bet, blind);
        if (result === false) {
            // Auto-pack player if they can't afford the bet
            console.log('Auto-packing player - insufficient chips for bet');
            return this.packPlayer(id);
        }
        this.getNextSlotForTurn(id);
        return this.getPlayers();
    };
        ...
        for (var player in players) {
            if (players[player].active) {
                ...
                if (players[player].playerInfo.chips < tableInfo.boot) {
                    console.log('Auto-packing player during boot collection - insufficient chips:', players[player].playerInfo.userName);
                    players[player].packed = true;
                    players[player].lastAction = "Auto-packed (insufficient chips)";
                    continue;
                }
    ...
    this.decideWinner = function(showCards) {
        ...
        console.log('💰 Pot amount awarded:', tableInfo.amount);
        ...
        DAL.db.collection('users').updateOne({
            ...
            if (err) {
                console.error('❌ Error updating winner chips:', err);
            } else {
                console.log('✅ Winner chips updated in database:', winnerObj.playerInfo.userName, '→', winnerObj.playerInfo.chips);
            }
        });
        if (winnerCard) {
